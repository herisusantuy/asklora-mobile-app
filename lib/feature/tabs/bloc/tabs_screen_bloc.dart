...
class TabScreenBloc extends Bloc<TabScreenEvent, TabScreenState> {
  TabScreenBloc() : super(const TabScreenState(currentIndexScreen: 0)) {
    ...
    on<BackButtonClickedTwice>(_onTwiceClickBackButton);
  }
  ...
  _onTwiceClickBackButton(
      BackButtonClickedTwice event, Emitter<TabScreenState> emit) async {
    emit(state.copyWith(isConfirmedToBack: true));
    await Future.delayed(const Duration(seconds: 3));
    emit(state.copyWith(isConfirmedToBack: false));
  }
}
